# 03 - Stream input and output

Demonstrates `std::cout`, `std::cin`, `std::getline`, `std::ws`, chaining, and `
`.

## Build and run with GCC / g++

```bash
g++ -std=c++20 -Wall -Wextra -pedantic main.cpp -o streams
./streams
```
...
```bash
clang++ -std=c++20 -Wall -Wextra -pedantic main.cpp -o streams
./streams
```
...
```cmd
cl /std:c++20 /W4 /EHsc main.cpp /Fe:streams.exe
streams.exe
```
...
Example input: `54` then `Mark Price`
