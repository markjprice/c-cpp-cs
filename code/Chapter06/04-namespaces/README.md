...
Shows how namespaces allow functions with the same name to coexist without conflict.

## Build and run with GCC / g++

```bash
g++ -std=c++20 -Wall -Wextra -pedantic main.cpp -o namespaces
./namespaces
```
...
```bash
clang++ -std=c++20 -Wall -Wextra -pedantic main.cpp -o namespaces
./namespaces
```
...
```cmd
cl /std:c++20 /W4 /EHsc main.cpp /Fe:namespaces.exe
namespaces.exe
```
